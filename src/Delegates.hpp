...
//Delegate that can be bound to by just ONE object
    ...
    public:
        ...
        template<typename T, typename... Args2>
        static Delegate CreateSP(const std::shared_ptr<T>& pObject,
                                 RetVal(T::*pFunction)(Args..., Args2...),
                                 Args2... args
                                )
        {
            Delegate handler;
            handler.Bind < SPDelegate <
            T, RetVal(Args...), Args2...>>(pObject, pFunction, std::forward<Args2>(args)...);
            return handler;
        }
...
//Delegate that can be bound to by MULTIPLE objects
    ...
    public:
        ...
        //Bind a member function with a shared_ptr object
        template<typename T, typename... Args2>
        inline DelegateHandle AddSP(std::shared_ptr<T> pObject,
                                    void(T::*pFunction)(Args..., Args2...),
                                    Args2&& ... args
                                   )
        {
